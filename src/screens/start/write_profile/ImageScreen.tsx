  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={{marginTop: 20}}>
        <View
          style={[styles.section, { marginTop: 30, height: 200, backgroundColor: 'white', flexWrap: 'wrap', gap: 8 },]}>

          <AddImageButton bubble={'필수'} />
          <AddImageButton bubble={'필수'} />
          <AddImageButton/>
          <AddImageButton/>
          <AddImageButton/>
        ...
        <View style={[styles.section, { marginTop: 15, marginBottom: 15 }]}>
          <View style={localStyles.container}>
            <GuideText
              text={'이목구비가 또렷하게 잘 나온 상반신 정면 사진을 필수로 두 장 등록해 주세요.'}
              bold={true} />
            <GuideText
              text={'나머지는 회원님의 매력을 어필할 수 있는 사진을 자유롭게 등록해 주세요. (단, 사물, 음식, 배경 사진 등 본인의 모습이 나오지 않았거나 중복된 사진은 불가)'}
              margin={4} />
            <GuideText
              text={'타인과 함께 찍은 사진은 개인정보보보를 위해 본인만 남기고 모자이크 처리나 스티커로 가려주세요.'}
              margin={4} />
            <GuideText
              text={'사진 등록 가이드를 지키기 않을 경우, 심사에서 반려될 수 있습니다.'}
              margin={4} />
          </View>
        </View>
...
function GuideText({ text, bold = false, margin = 0 }: GuideProps) {
  return (
    <View style={{flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-start', marginTop: margin}}>
